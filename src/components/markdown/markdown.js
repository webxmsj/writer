...
// import 'https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.0/MathJax.js?config=TeX-AMS-MML_HTMLorMML'
...
export default {
  ...
  methods: {
    init() {
      ...
      this.indexLenth = parseInt(this.editorHeight / 28, 0) + 1
    },
    handlePaste(e) { // 粘贴图片
      const {
        clipboardData = {},
    ...
    addImageClickLintener() { // 监听查看大图
      ...
      setTimeout(() => {
        ...
        for (let i = 0, len = this.imgs.length; i < len; i += 1) {
          this.imgs[i].onclick = () => {
  ...
  watch: {
    ...
    value() {
      ...
      this.indexLenth = parseInt(this.scrollHeight / 28, 0) + 1
